...
public class InBodyReadingTests
{
    ...
    public void Ctor_assigns_id_fields_recorder_and_created_at()
    {
        ...
        var r = new InBodyReading(swimmerId, date, 180m, 74m, 12.8m, 42.1m, 55.3m, 1.35m, 1.07m, recordedBy);
    ...
    {
        var recordedBy = Guid.NewGuid();
        var r = new InBodyReading(Guid.NewGuid(), new DateOnly(2024, 1, 1), 1m, 1m, 1m, 1m, 1m, 1m, 1m, recordedBy);
        var createdAt = r.CreatedAt;

        r.Update(new DateOnly(2024, 8, 12), 182m, 75.5m, 14.1m, 41.2m, 50.5m, 1.33m, 1.06m);

        Assert.Equal(new DateOnly(2024, 8, 12), r.ReadingDate);
        Assert.Equal(182m, r.HeightCm);
        Assert.Equal(50.5m, r.WaterPct);
