...
public class InBodyReadingServiceTests
{
    ...
    public async Task List_maps_rows()
    {
        ...
        repo.Setup(r => r.ListBySwimmerAsync(sw, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new List<InBodyReading> { new(sw, new DateOnly(2024, 10, 4), 180m, 74m, 12.8m, 42.1m, 55.3m, 1.35m, 1.07m, Guid.NewGuid()) });
    ...
    public async Task Update_applies_when_owned_and_returns_null_when_foreign_or_missing()
    {
        var (svc, repo) = Build();
        var sw = Guid.NewGuid();
        var owned = new InBodyReading(sw, new DateOnly(2024, 1, 1), 1m, 1m, 1m, 1m, 1m, 1m, 1m, Guid.NewGuid());
        repo.Setup(r => r.GetTrackedAsync(owned.Id, It.IsAny<CancellationToken>())).ReturnsAsync(owned);
        var foreign = new InBodyReading(Guid.NewGuid(), new DateOnly(2024, 1, 1), 1m, 1m, 1m, 1m, 1m, 1m, 1m, Guid.NewGuid());
